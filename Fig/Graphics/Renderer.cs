...
namespace Fig.Graphics
{
    public class Renderer
    {
        ...
        public BlendState BlendState
        {
            ...
            {
                _blendState = value;
                _blendFunction = _blendState switch
                {
                    BlendState.None => BlendNone,
                    BlendState.Alpha => BlendAlpha,
                    BlendState.Additive => BlendAdditive,
                    _ => BlendNone
                };
            }
        }
        ...
        public void DrawPixel(int x, int y, Color color) =>
            _canvas[x, y] = _blendFunction(color, _canvas[x, y]);
        public void DrawLine(int x0, int y0, int x1, int y1, Color color)
        {
            ...
            {
                if (steep) 
                    _canvas[y, x] = _blendFunction(color, _canvas[y, x]);
                else 
                    _canvas[x, y] = _blendFunction(color, _canvas[x, y]);
                error += errorInc;
                if (error > dx)
        ...
        public void DrawRectangle(int x0, int y0, int x1, int y1, Color color)
        {
            ...
            for (x = x0; x <= x1; x++)
            {
                ...
                _canvas[index] = _blendFunction(color, _canvas[index]);
                y = y1;
                index = x + y * _canvas.Width;
                _canvas[index] = _blendFunction(color, _canvas[index]);
            }
        ...
        public void DrawCircle(int x, int y, int radius, Color color)
        {
            ...
            while (i <= j)
            {
                _canvas[x + i, y + j] = _blendFunction(color, _canvas[x + i, y + j]);
                _canvas[x + j, y + i] = _blendFunction(color, _canvas[x + j, y + i]);
                _canvas[x + j, y - i] = _blendFunction(color, _canvas[x + j, y - i]);
                _canvas[x + i, y - j] = _blendFunction(color, _canvas[x + i, y - j]);
                _canvas[x - i, y - j] = _blendFunction(color, _canvas[x - i, y - j]);
                _canvas[x - j, y - i] = _blendFunction(color, _canvas[x - j, y - i]);
            ...
            for (var y = minY; y <= maxY; y++)
            {
                for (var x = minX; x <= maxX; x++)
                {
                    ...
                    if (w0 >= 0f && w1 >= 0f && w2 >= 0f)
                    {
                        ...
                        _canvas[x, y] = _blendFunction(interpolatedColor, _canvas[x, y]);
                    }
                }
            }
        }
        ...
        public void FillCircle(int x, int y, int radius, Color color)
        {
            ...
            for (var ys = -radius; ys <= radius; ys++)
                for (var xs = -radius; xs <= radius; xs++)
                    if (xs * xs + ys * ys <= r2)
                        _canvas[x + xs, y + ys] = _blendFunction(color, _canvas[x + xs, y + ys]);
        }
        ...
        public void DrawText(string text, int x, int y, Color color)
        {
            ...
            foreach (var c in text)
            {
                ...
                for (i = 0; i < 8; i++)
                {
                    ...
                    for (j = 0; j < 8; j++)
                        if ((glyph & (1 << j)) != 0)
                            _canvas[x + index + j, y + i] = _blendFunction(color, _canvas[x + index + j, y + i]);
                }
                index += 8;
            }
        }
                            ...
                            for (k = 0; k < scale; k++)
                            {
                                ...
                                for (l = 0; l < scale; l++)
                                {
                                    ...
                                    if (xIndex >= _canvas.Width)
                                        break;
                                    _canvas[xIndex, yIndex] = _blendFunction(color, _canvas[xIndex, yIndex]);
                                }
                            }
                        }
                    }
        ...
        public void DrawCanvas(Canvas canvas, int x, int y, Color color)
        {
            ...
            for (int i = y; i < yEnd; i++)
            {
                ...
                for (int j = x; j < xEnd; j++)
                {
                    ...
                    _canvas[j, i] = _blendFunction(c, _canvas[j, i]);
                }
            }
        }
        public void DrawCanvas(Canvas canvas, int x, int y, int width, int height, Color color)
        {
            ...
            for (int i = y; i < yEnd; i++)
            {
                ...
                for (int j = x; j < xEnd; j++)
                {
                    ...
                    c = (_samplerFunction(canvas, canvasX1, canvasY1, canvasX2, canvasY1, uLerp, vLerp).UBGRA & color.UBGRA);
                    _canvas[j, i] = _blendFunction(c, _canvas[j, i]);
                }
            }
        }
        public void DrawCanvas(Canvas canvas, int srcX, int srcY, int srcW, int srcH, int dstX, int dstY, int dstW, int dstH, Color color)
        {
            ...
            for (int i = dstY; i < yEnd; i++)
            {
                ...
                for (int j = dstX; j < xEnd; j++)
                {
                    ...
                    c = (_samplerFunction(canvas, canvasX1, canvasY1, canvasX1 + 1, canvasY1 + 1, uLerp, vLerp).UBGRA & color.UBGRA);
                    _canvas[j, i] = _blendFunction(c, _canvas[j, i]);
                }
            }
        }
        ...
        private delegate Color BlendFunction(Color src, Color dst);
        private Color BlendNone(Color src, Color dst)
        {
            return src;
        }
        private Color BlendAlpha(Color src, Color dst)
        {
            return Color.Blend(dst, src);
        }
        private Color BlendAdditive(Color src, Color dst)
        {
            return Color.Add(src, dst);
        }
